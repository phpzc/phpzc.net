...
@section('head')
    <script>
        function addBook() {
            $.post('/book/addBook', '{{ $StoryDetail2 ?? ""}}', function () {
                alert('success');
            }, 'json');
        }
            ...
            <?php if(!empty($book_cookie)){
                ...
                <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
                    <div class="main-box clearfix profile-box-contact">
                        <div class="main-box-body clearfix">
                            <div class="profile-box-header gray-bg clearfix">
                                <img src="{{ $bv['value']['face_image_url'] ?? '' }}" alt="" class="profile-img img-responsive"/>
                                <h2>{{ $bv['value']['author'] ?? '' }}</h2>
                                <div class="job-position">
    {{ $bv['value']['title'] ?? '' }}
                                </div>
                                <ul class="contact-details">
                                    <li>
                                        <button class="btn btn-danger" onclick="addBook2('{{ $bv["value"]["title"] ?? "" }}',this)">
                                            删除书架
                                        </button>
                                    </li>
