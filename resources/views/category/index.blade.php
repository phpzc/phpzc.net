        ...
        <form action="/category/add" id="form1" method="post">
            ...
            @foreach($result as $v)
                ...
                @if ( $v->pid == 0)
                    ...
                    <div class="top_{{ $v->id ?? '' }} category">
                        <div><a href="javascript:;" onclick="topshow('top_{{ $v->id ?? ''}}',this)">★</a></div>
                        <div><input type="text" value="{{ $v->name ?? ''}}" id="top_{{ $v->id  ?? '' }}" /></div>
                        <div><a href="javascript:;" onclick="add('{{ $v->id  ?? '' }}')">@添加子类</a>&nbsp;&nbsp;&nbsp;<a href="javascript:;" onclick="edit('{{ $v->id  ?? '' }}')">@提交编辑</a>&nbsp;&nbsp;&nbsp;<a href="#" onclick="del('{{ $v->id  ?? '' }}')">删除分类</a></div>
                    </div>
                @else
                    <div class="top_{{ $v->pid }}_child category" style="display:block;">
                        <span>————————<input type="text" name="" value="{{ $v->name ?? ''}}" id="top_{{ $v->id  ?? '' }}"/></span>
                        <span><a href="javascript:;" onclick="edit('{{ $v->id  ?? '' }}')">@提交编辑</a>&nbsp;&nbsp;&nbsp;<a href="#" onclick="del('{{ $v->id  ?? ''}}')">删除分类</a></span>
                    </div>
